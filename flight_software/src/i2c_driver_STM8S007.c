...
{
    if (num_bytes > 1) I2C_AcknowledgeConfig(I2C_ACK_CURR); /* Need to ack the bytes received so that we keep reading */
    else I2C_AcknowledgeConfig(I2C_ACK_NONE); /* Send a nack on next byte received */
    ...
    /* Nack to next byte received */
    I2C_AcknowledgeConfig(I2C_ACK_NONE); /* Send a nack on next byte received to let slave know we are done */
    while(!I2C_CheckEvent(I2C_EVENT_MASTER_BYTE_RECEIVED)) continue;
